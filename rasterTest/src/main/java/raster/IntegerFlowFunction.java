...
public class IntegerFlowFunction {

    private static final Logger LOGGER = LoggerFactory.getLogger(IntegerFlowFunction.class);


    public static void applyObstructions(int unobstructedFlow[], TileType neighbourTiles[]) {
    ...
    public static double[] distributeFlowWithNoObstructions(
        ...
        return Doubles.toArray(Arrays.stream(FlowDirections.values())
                .map(FlowDirections::getAngleInRadians)
                .map(input -> distributionFunction.apply(angleInRadians, input))
                .map(input -> input * flow)
                .collect(Collectors.toList()));
    }

    public static int[] distributeFlow(double flow, double angleInRadians) {
        return distributeFlowWithNoObstructions(flow, angleInRadians, createTriangularDissipationFunction(TRIANGLE_BASE));
    }

    public static int[] distributeFlow(double flow, double angleInRadians, DissipationFunction dissipationFunction) {
