...
public class BooleanEncodingIterator {
    ...
    static void placeSource(BooleanEncoding booleanEncoding) {
...
//        }
        ...
        double angle = 0;
        FlowDirections directions[] = FlowDirections.values();
...
//                    updatedFlowRaster[row + directions[i].getRowShift()][column + directions[i].getColumnShift()]);
    ...
    static double[] dissipateEnergy(Complex center, TileType neighbours[]) {
        FlowDirections direction = getDirection(center.getArgument());
...
//        distributed[0] = Complex.ZERO;
        ...
        FlowDirections startDirection = getDirection(direction, -2);
...
//        LOG.info("Fraction3: {}", fraction);
    ...
    static Complex[] distributeEnergy(Complex flow, TileType neighbours[]) {
        ...
        FlowDirections direction = getDirection(flow.getArgument());
        int cell = direction.ordinal();
...
//        return energyStop - energyStart;
    ...
    private static FlowDirections getDirection(FlowDirections direction, int shift) {
        int temp = direction.ordinal() + shift;
        if (temp < 0) {
            temp += FlowDirections.values().length;
        } else if (temp >= FlowDirections.values().length) {
            temp -= FlowDirections.values().length;
        }
        return FlowDirections.values()[temp];
    }


    static FlowDirections getDirection(double pAngle) {
        pAngle = fixAngle(pAngle);
        if (pAngle < Math.PI / 8 || pAngle >= 7 / 4 * Math.PI) {
            return FlowDirections.EAST;
        }

        int directionCounter = 1;
        for (double d = Math.PI / 4; d < 2 * Math.PI - Math.PI / 8; d += Math.PI / 4) {
            if (pAngle < d + Math.PI / 8) {
                return FlowDirections.values()[directionCounter];
            }
            ++directionCounter;
        }
